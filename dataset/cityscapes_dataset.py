...
class CityscapesDataSetWithLabel(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True,
                 mirror=True, ignore_label=255, set='val'):
        self.root = root
        self.list_path = list_path
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(self.root,
                                  'gtFine/%s/%s' % (self.set, name.replace('leftImg8bit', 'gtFine_labelIds')))
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
if __name__ == '__main__':
    dst = CityscapesDataSetWithLabel(root='/data3/chenlin/data/cityscapes', list_path='./cityscapes_list/train.txt',
                                     max_iters=4e4, set='train', mean=[0, 0, 0])
